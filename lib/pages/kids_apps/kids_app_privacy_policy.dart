...
import '../privacy_policy/policy_sections.dart';
import 'kids_app_configs.dart';

class KidsAppPrivacyPolicyScreen extends ConsumerWidget {
  final String appId;

  const KidsAppPrivacyPolicyScreen({super.key, required this.appId});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final config = kKidsAppConfigs[appId.toLowerCase()];
    
    // Fallback if appId is not found
    final appName = config?.name ?? 'Our App';
    final appDescription = config?.description ?? 'an educational application for children.';
    ...
    return Scaffold(
      body: Stack(
        children: [
                  ...
                  PageWrapper(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          'Effective Date: July 21, 2024  ·  Last Updated: July 21, 2024',
                          style: GoogleFonts.outfit(
                            color: secColor,
                            fontSize: 15,
                          ),
                        ...
                        _PolicyContent(
                          ...
                          textColor: textColor,
                          secColor: secColor,
                          useDetailedPolicy: config?.useDetailedPolicy ?? false,
                        ),
                      ],
                    ),
                  ),
          ...
          CommonHeader(title: '$appName Privacy'),
        ],
      ),
    );
  }
...
class _PolicyContent extends StatelessWidget {
  ...
  const _PolicyContent({
    ...
    required this.useDetailedPolicy,
  });

  @override
  Widget build(BuildContext context) {
    final sections = useDetailedPolicy
        ? getDetailedPolicySections(isForKids: true)
        : [
            _PolicySection(
            ...
            _PolicySection(
              ...
              body:
                  '**$appName does not collect any personal information from children.**\n\n'
            ...
            const _PolicySection(
              ...
              body:
                  'All app progress and settings are processed and stored **locally '
                  'on your device**. No data related to the child\'s interactions '
                  'is transmitted to external servers.',
            ),
            _PolicySection(
              title: '4. Advertisements',
              body:
                  '$appName may display advertisements to support its ongoing development. '
                  ...
                  'content of the app to avoid any confusion for the child.',
            ),
            const _PolicySection(
    ...
    return Column(
      ...
      children: sections
          .map((s) => _SectionWidget(
                section: s is PolicySectionModel
                    ? _PolicySection(title: s.title, body: s.body)
                    : s as _PolicySection,
                textColor: textColor,
                secColor: secColor,
              ))
